...
func TestMergeStyles(t *testing.T) {
	tests := []struct {
		...
		{
			name:   "overflow-y merged",
			base:   css.Style{OverflowY: "visible"},
			inline: css.Style{OverflowY: "hidden"},
			verify: func(t *testing.T, r *css.Style) { assert.Equal(t, "hidden", r.OverflowY) },
		},
		{
