...
- [x] `border-style` rendering variations (§5.5.17) - `dotted`, `dashed`, `double`, `groove`, `ridge`, `inset`, `outset` rendered via `emitBorderSide` in `render/paint.go`
...
- [x] `pt` - points, 1pt = 1/72in (§6.1)
- [x] `pc` - picas, 1pc = 12pt (§6.1)
- [x] `in` - inches (§6.1)
...
- [x] `rgb()` - color function (§6.3 — supports integer and percentage forms with clamping)
